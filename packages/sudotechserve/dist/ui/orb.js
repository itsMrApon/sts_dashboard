...
/** R3F sometimes measures 0×0 on first paint; sync canvas to parent box after layout & resize. */
function SyncCanvasToParent() {
    ...
    useEffect(() => {
        ...
        const apply = () => {
            ...
            // `true` updates canvas CSS width/height; `false` only resizes the drawing buffer and can
            // leave a 0×0 canvas visible after a bad first measure (common on hard reload).
            gl.setSize(w, h, true);
            invalidate();
        };
        apply();
...
export function Orb({ colors = ["#CADCFC", "#A0B9D1"], colorsRef, resizeDebounce = 100, seed, agentState = null, volumeMode = "auto", manualInput, manualOutput, inputVolumeRef, outputVolumeRef, getInputVolume, getOutputVolume, className, }) {
    return (_jsx("div", { className: className ?? "relative h-full w-full", children: _jsxs(Canvas, { className: "block h-full w-full min-h-[1px] min-w-[1px] touch-none", resize: { debounce: resizeDebounce, scroll: true }, gl: {
                alpha: true,
                antialias: true,
                ...
                gl.setSize(w, h, true);
                inv();
            }, children: [_jsx(SyncCanvasToParent, {}), _jsx(Scene, { colors: colors, colorsRef: colorsRef, seed: seed, agentState: agentState, volumeMode: volumeMode, manualInput: manualInput, manualOutput: manualOutput, inputVolumeRef: inputVolumeRef, outputVolumeRef: outputVolumeRef, getInputVolume: getInputVolume, getOutputVolume: getOutputVolume })] }) }));
}
function Scene({ colors, colorsRef, seed, agentState, volumeMode, manualInput, manualOutput, inputVolumeRef, outputVolumeRef, getInputVolume, getOutputVolume, }) {
    const { gl } = useThree();
    const circleRef = useRef(null);
    const initialColorsRef = useRef(colors);
    const targetColor1Ref = useRef(new THREE.Color(colors[0]));
    const targetColor2Ref = useRef(new THREE.Color(colors[1]));
    const animSpeedRef = useRef(0.05);
    const perlinNoiseTexture = useMemo(() => {
        // Avoid external network/CSP dependency on hard reloads.
        // Any repeatable grayscale noise texture works with the shader's sampling.
        const size = 128;
        const rand = splitmix32(seed ?? Math.floor(Math.random() * 2 ** 32));
    ...
    useFrame((_, delta) => {
        ...
        // Keep motion subtle and close to the original visual pace.
        const targetSpeed = 0.05 + (1 - Math.pow(curOutRef.current - 1, 2)) * 0.24;
        animSpeedRef.current += (targetSpeed - animSpeedRef.current) * 0.08;
    ...
    return (_jsxs("mesh", { ref: circleRef, children: [_jsx("circleGeometry", { args: [3.5, 64] }), _jsx("shaderMaterial", { uniforms: uniforms, fragmentShader: fragmentShader, vertexShader: vertexShader, transparent: true })] }));
}
